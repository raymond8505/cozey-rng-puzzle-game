...
- **Drag-to-pry** (armed crowbar): placed tiles get HTML ghost `motion.div`s in `.pry-layer` (`Board.tsx`), dropped on `"window"`, `"queue"`, or an empty `"cell:<n>"` (one-drag relocate). Queue/cell drops reuse the normal pipeline — `playCrowbar` then `PARK`/`PLACE` as two dispatches in `GameScreen.onPry` — no dedicated lift-and-route reducer action. The parking tray (`ParkingArea` in `src/ui/machine/`) must advertise `data-drop` during an armed pry explicitly; `canPark` alone only opens during routing (pinned in `ParkingArea.test.tsx`).
- **An armed crowbar is spent by ANY tile play**, not just the pry: arming is UI-store state (`pendingCrowbar`) the reducer hasn't seen, so the store's `dispatch` applies `FORFEIT_CROWBAR` (discard, no lift, does NOT claim `cardPlayedThisTurn` — that would reject the abandoning `PLACE_FROM_QUEUE`) before `DRAW`/`PLACE_FROM_QUEUE` while armed. Hand and Draw are UI-gated while armed; place-from-queue deliberately is not.
...
- **Parking lives on the machine** (`ParkingArea` between window and card slot): parked tiles render littered via UI-only scatter poses (`src/ui/machine/scatter.ts` — ±25° tilt off upright, band-spread x). Tilts stay shallow **by directive**: a parked tile must telegraph its board alignment without being picked up — no sideways tumbles. Pose data must never enter `GameState`. On the draggable, the tumble rides motion's `style={{ rotate }}` (motion owns `transform`); the scatter wrapper centers with negative margins, not `translate(-50%,-50%)`.
...
- **No decorative background art under the board svg**: backgrounds paint below children, so rivet dots on the board panel pop in and out as tiles cover the corner cells. The chassis carries the rivet motif (plus the faded cherry wash rising from its bottom edge — the controls row has no plate of its own). The board panel lives on `.board-stage` (`.board-frame` is a bare wrapper); the artwork fills it edge to edge, cropped by `clip-path: inset(0 round var(--radius))` on the svg — clip-path, not overflow, so pry ghosts can leave the board mid-drag. No padding/border on the stage, or the overlay alignment invariant breaks.
